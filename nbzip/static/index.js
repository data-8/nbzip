require([
    ...
    function Zipper(baseUrl) {
        this.baseUrl = baseUrl;
        this.redirectUrl = baseUrl + 'files/notebook.zip';
        this.callbacks = {};
    }

    Zipper.prototype.addHandler = function(event, cb) {
    ...
    Zipper.prototype._emit = function(event, data) {
    ...
    Zipper.prototype.start = function() {
        var syncUrl = this.baseUrl + 'zip-download/api?'+ $.param({
    ...
    function ZipperView(termSelector, progressSelector, termToggleSelector) {
    ...
    ZipperView.prototype.setTerminalVisibility = function(visible) {
        if (visible) {
    ...
    ZipperView.prototype.setProgressValue = function(val) {
        this.$progress.attr('aria-valuenow', val);
        this.$progress.css('width', val + '%');
    };

    ZipperView.prototype.getProgressValue = function() {
        return parseFloat(this.$progress.attr('aria-valuenow'));
    };

    ZipperView.prototype.setProgressText = function(text) {
        this.$progress.children('span').text(text);
    };

    ZipperView.prototype.getProgressText = function() {
        return this.$progress.children('span').text();
    };

    ZipperView.prototype.setProgressError = function(isError) {
        if (isError) {
    ...
    ZipperView.prototype._fitTerm = function() {
    ...
    var gsv = new ZipperView(
        '#status-details',
        '#status-panel-title',
        '#status-panel-toggle'
    );

    gs.addHandler('zipping', function(data) {
    ...
    gs.addHandler('finished', function(data) {
        ...
        gsv.setProgressText('Zip finished, redirecting...');
        window.open(gs.redirectUrl, '_blank');
